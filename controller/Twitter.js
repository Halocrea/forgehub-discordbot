...
class Twitter {
	...
	broadcastTweetURL (tweetURL) {
		const guilds = new Guilds().all().filter(g => !!g?.tweetsChanId)
		guilds.forEach((g, i) => {
			setTimeout(async () => {
				try {
			...
			}, i * 30000) // to avoid Discord API rate limit
		})
	}
	...
	static async getLatestTweet (interaction) {
		try {
			const guild   = new Guilds().getById(interaction.guildId)
			const token   = await Twitter.authenticate()
			const headers = { Authorization: `Bearer ${token}` }
			const params  = {
				'exclude_replies': true,
			...
			if (data?.length < 1) {
				...
				interaction.reply({
					content  : $t.get('errorCantFindTweet'),
					ephemeral: true
				})
			} else {
				interaction.reply({
					content  : `https://twitter.com/${data[0].user.screen_name}/status/${data[0].id_str}`,
					ephemeral: true
				})
			}
